	...
	try{
		...
		return query.exec()
			.then(foundManagement => {
				...
				const incomes = foundManagement.incomes;
				const outgoings = foundManagement.outgoings;
				...
				const balance = foundManagement.amount + total - outgoing;
				...
				const result = {
					...
					outgoing,
					total,
					firstAmount: foundManagement.amount,
					balance,
					grossProfit: balance - foundManagement.amount,
				};
	...
	try{
		...
		return query.exec()
			.then(foundManagement => {
				if (!foundManagement)
					return next({status: 404, error: 'not_found', error_description: 'there is no open journal at the moment'});
				return res.status(201).send(foundManagement);
			})
			.catch(next);
